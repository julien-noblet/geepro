...
static int xyz_gui(geepro *gep, const char *chip_name, const char *family)
{
    ...
    gui_xml_build(GUI_XML(GUI(gep->gui)->xml), (char *)iface_get_xml_path(gep->ifc), (char *)"info,notebook", xyz_if_attr, gep->shared_geepro_dir);
...
int xyz_api(void *g, en_hw_api func, int val, void *ptr)
{
	...
	case HW_SET_CHIP: return xyz_gui( GEEPRO(ptr), iface_get_chip_name(GEEPRO(ptr)->ifc), iface_get_chip_family(GEEPRO(ptr)->ifc) );
	// iface
