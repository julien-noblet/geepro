...
static void willem_set_gui_main(geepro *gep, const char *chip_name, const char *family, const char *programmer)
{
    ...
    gui_xml_build(GUI_XML(GUI(gep->gui)->xml), (char *)iface_get_xml_path(gep->ifc), (char *)"info,notebook", willem_if_attr, gep->shared_geepro_dir);
...
int willem_40_hardware_driver(void *g,en_hw_api funct, int val, void *ptr)
{
	...
	case HW_SET_CHIP: return willem_gui_init(ptr, iface_get_chip_name(GEEPRO(ptr)->ifc), iface_get_chip_family(GEEPRO(ptr)->ifc), "willem_40");
...
int willem4_hardware_driver(void *g,en_hw_api funct, int val, void *ptr)
{
	...
	case HW_SET_CHIP: return willem_gui_init(ptr, iface_get_chip_name(GEEPRO(ptr)->ifc), iface_get_chip_family(GEEPRO(ptr)->ifc), "willem_pcb3");
	case HW_TEST:	  return willem_test_connection();
	case HW_TEST_CONTINUE:	  return 0;
	case HW_RESET:    return willem_reset();
	case HW_SET_VCC:  return willem_set_vcc_voltage(val);
...
int willempro2_hardware_driver(void *g, en_hw_api funct, int val, void *ptr)
{
	...
	case HW_SET_CHIP: return willem_gui_init(ptr, iface_get_chip_name(GEEPRO(ptr)->ifc), iface_get_chip_family(GEEPRO(ptr)->ifc), "willem_pro2");
	case HW_TEST:	  return willem_test_connection();
	case HW_TEST_CONTINUE:	  return 0;
	case HW_RESET:    return willem_reset();
	case HW_SET_VCC:  return willem_set_vcc_voltage(val);
